...
if __name__ == '__main__':
  ...
  for i in range(repeat_n):
    ...
    np.save('/home/chunx/raid_storage/LFBS_data/'+str(length)+'_'+str(i)+'.npy',sig)
    with open('/home/chunx/raid_storage/LFBS_data/'+str(length)+'ref_'+str(i)+'.txt','w') as f:
      f.write(ref)
    ###
    ...
    for num in bstest:
      ...
      acc['BS'+str(num)][i] = ali[0].score/ali[0].shape[1]
      match_len['BS'+str(num)][i] = ali[0].score
    ### LFBS
    for num in lfbstest:
      start = time.time()
      ...
      fasta = base2kmer(output.numpy(),5,1,1)
      ali = aligner.align(ref,fasta)
      acc['LFBS'+str(num)][i] = ali[0].score/ali[0].shape[1]
      match_len['LFBS'+str(num)][i] = ali[0].score
    ### Viterbi
    start_t = time.time()
    ...
    ali = aligner.align(ref,fasta)
    acc['viterbi'][i] = ali[0].score/ali[0].shape[1]
    match_len['viterbi'][i] = ali[0].score
